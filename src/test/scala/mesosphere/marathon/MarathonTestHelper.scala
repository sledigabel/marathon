...
import mesosphere.marathon.state.{ AppDefinition, MarathonStore, MarathonTaskState, Timestamp }
...
trait MarathonTestHelper {
  ...
  def createTaskTracker(
    ...
    val protoStore = new MarathonStore[MarathonTaskState](
      store = store,
      metrics = metrics,
      newState = () => MarathonTaskState(MarathonTask.newBuilder().setId("task_a").build()),
