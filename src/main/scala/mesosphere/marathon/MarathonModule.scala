...
class MarathonModule(conf: MarathonConf, http: HttpConf, zk: ZooKeeperClient)
  ...
  def provideTaskTracker(store: PersistentStore, metrics: Metrics, conf: MarathonConf): TaskTracker = {
    new TaskTracker(
      new MarathonProtoStore[MarathonTask, MarathonTask, MarathonTaskConverter](
        store = store,
        converter = new MarathonTaskConverter,
        metrics = metrics,
        newState = () => MarathonTask.newBuilder().setId(UUID.randomUUID().toString).build(),
  ...
  def provideMigration(
    ...
    new Migration(store, appRepo, groupRepo, taskTracker, config, metrics)
  }
