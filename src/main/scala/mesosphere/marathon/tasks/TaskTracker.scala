...
import mesosphere.marathon.state.{EntityStore, PathId, StateMetrics, Timestamp}
...
class TaskTracker @Inject() (
    store: EntityStore[MarathonTask],
    config: MarathonConf,
  ...
  private[tasks] def fetchFromState(id: String): Option[MarathonTask] = timedRead {
    Await.result(store.fetch(id), timeout)
  }
