...
// TODO: newState is used in various ways. maybe require an 'empty' function?
class MarathonProtoStore[S, M <: Message, C <: StateConverter[M, S]](
  ...
  def modify(key: String)(f: Update): Future[S] = lockManager.executeSequentially(key) {
    val res = store.load(prefix + key).flatMap {
      case Some(entity) =>
        ...
        val updatedEntity = entity.withNewContent(converter.toProtoByteArray(updated))
        bytesWritten.update(updatedEntity.bytes.length)
        store.update(updatedEntity)
      case None =>
        val created = converter.toProtoByteArray(f(() => newState()))
  ...
  private def stateFromBytes(bytes: Array[Byte]): S = converter.fromProto(bytes)
}
